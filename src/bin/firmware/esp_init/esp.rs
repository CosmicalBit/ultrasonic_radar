...
use super::wifi::{Connected, NetworkError, Wifi};
...
impl Esp<I2C_TYPE> {
    pub async fn init(spawner: Spawner) -> Self {
        ...
        let i2c = I2C_TYPE::new(peripherals.I2C0, I2cConfig::default())
            .unwrap()
        ...
        let wifi = Wifi::new(controller, interfaces, spawner);
        let wifi = wifi.connect().await;
...
impl Esp<I2C_TYPE> {
    ...
    pub async fn send_udp_internal(&self, data: impl AsRef<[u8]>) -> Result<(), NetworkError> {
        self.wifi.send_udp(data).await
    }
}
